#!/bin/bash
source tipc/train_infer_python/common_func.sh
        ...
        if [ ${use_gpu} = "False" ] || [ ${use_gpu} = "cpu" ]; then
            for use_mkldnn in ${use_mkldnn_list[*]}; do
                ...
                for threads in ${cpu_threads_list[*]}; do
                    for batch_size in ${batch_size_list[*]}; do
                        ...
                        set_output_img_params1=$(func_set_params "${save_export_key}" "${_ouput_img}")
                        command="${_python} ${_script} ${use_gpu_key}=${use_gpu} ${use_mkldnn_key}=${use_mkldnn} ${set_cpu_threads} ${set_model_dir} ${set_batchsize} ${set_infer_data} ${set_benchmark} ${set_infer_params1} ${set_output_img_params1} > ${_save_log_path} 2>&1 "
                        eval $command
            ...
            for precision in ${precision_list[*]}; do
                ...
                for batch_size in ${batch_size_list[*]}; do
                    ...
                    set_infer_params1=$(func_set_params "${infer_key1}" "${infer_value1}")
                    set_output_img_params1=$(func_set_params "${save_export_key}" "${_ouput_img}")
                    command="${_python} ${_script} ${use_gpu_key}=${use_gpu} ${set_precision} ${set_model_dir} ${set_batchsize} ${set_infer_data} ${set_benchmark} ${set_infer_params1} ${set_output_img_params1} > ${_save_log_path} 2>&1 "
                    eval $command
                    last_status=${PIPESTATUS[0]}
                    eval "cat ${_save_log_path}"
                    status_check $last_status "${command}" "${status_log}"
...
if [ ${MODE} = "whole_infer" ] || [ ${MODE} = "klquant_whole_infer" ]; then
    ...
    for infer_mode in ${infer_mode_list[*]}; do
        ...
        func_inference "${python}" "${inference_py}" "${save_export_model_dir}" "${LOG_PATH}" "${infer_img_dir}" "${save_export_value}" ${is_quant}
        Count=$((${Count} + 1))
    done
else
    IFS="|"
        ...
        for autocast in ${autocast_list[*]}; do
            for trainer in ${trainer_list[*]}; do
                ...
                if [ ${run_export} != "null" ]; then
                    ...
                    save_export_model_dir="${save_export_value}/${model_name}"
                    eval $env
                    func_inference "${python}" "${inference_py}" "${save_export_model_dir}" "${LOG_PATH}" "${train_infer_img_dir}" "${save_export_value}" "${flag_quant}" 
