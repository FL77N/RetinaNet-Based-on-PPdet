...
class RetinaNetPostProcess(object):
    ...
    def __call__(self, pred_scores, pred_boxes, anchors, inputs):
        ...
        if isinstance(inputs["im_shape"], list):
            inputs["im_shape"] = paddle.concat(inputs["im_shape"])
        
        if isinstance(inputs["scale_factor"], list):
            inputs["scale_factor"] = paddle.concat(inputs["scale_factor"])

        if "scale_factor" in inputs:
            scale_factor_wh = paddle.concat([inputs["scale_factor"][:, 1:2], 
                                             inputs["scale_factor"][:, 0:1]], axis=-1)
            img_wh = paddle.concat([inputs["im_shape"][:, 1:2],
                                    inputs["im_shape"][:, 0:1]],axis=-1)
        else:
            scale_factor_wh = paddle.ones([len(inputs["im_shape"]), 2]).astype("float32")
            img_wh = paddle.concat([inputs["im_shape"][:, 1:2],
                                    inputs["im_shape"][:, 0:1]],axis=-1)
